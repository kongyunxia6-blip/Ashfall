    ...
    public static class UndergroundSpaceV1Builder
    {
        ...
        public static void Build()
        {
            ...
            spaceGen.bands = new[]
            {
                ...
                {
                    bandName = "Shallow", minDepth = 5, maxDepth = ShallowMax,
                ...
                new UndergroundSpaceBand
                {
                    bandName = "Mid", minDepth = MidMin, maxDepth = MidMax,
                ...
                new UndergroundSpaceBand
                {
                    bandName = "Deep", minDepth = DeepMin, maxDepth = DeepMax,
            ...
            oreGen.bands = new[]
            {
                new OreDepthBand
                {
                    bandName = "Shallow", minDepth = ShallowMin, maxDepth = ShallowMax,
            ...
            MakeSign(signGo.transform, "── Shallow 浅层 (y5..21) ──", digGrid.GridToWorld(8, ShallowMax), new Color(0.7f, 1f, 0.7f));
            MakeSign(signGo.transform, "── Mid 中层 (y22..43) ──", digGrid.GridToWorld(8, MidMax), new Color(1f, 1f, 0.7f));
            MakeSign(signGo.transform, "── Deep 深层 (y44..62) ──", digGrid.GridToWorld(8, DeepMax), new Color(1f, 0.75f, 0.7f));
            ...
            Debug.Log($"[DEV-008] 地下探索空间测试场景已搭建：{ScenePath}\n" +
                      $"seed={digGrid.seed}，网格 {GridWidth}x{GridDepth}，空间带 Shallow({5}..{ShallowMax})/" +
                      $"Mid({MidMin}..{MidMax})/Deep({DeepMin}..{DeepMax})；" +
